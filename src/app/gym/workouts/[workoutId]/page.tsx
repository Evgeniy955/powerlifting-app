...
export default async function GymWorkoutPage({ params }: { params: Promise<{ workoutId: string }> }) {
  ...
  return (
    <main className="min-h-[calc(100vh-3.5rem)] bg-bg py-6 text-text-primary">
      <div className="mx-auto mb-2 flex max-w-5xl items-center justify-between px-4">
        <Link href={`/gym/weeks/${workout.weekId}`} className="text-sm text-text-secondary">
          ← Неделя {workout.week.weekNumber}
        </Link>
        <Link
      ...
      <div className="mb-4 text-center">
        <div className="flex items-center justify-center gap-3">
          {workout.prevDay ? (
            <Link
              href={`/gym/workouts/${workout.prevDay.id}`}
          ...
          <h1 className="font-display text-lg uppercase tracking-wide">День {workout.dayNumber}</h1>
          {workout.nextDay ? (
            <Link
              href={`/gym/workouts/${workout.nextDay.id}`}

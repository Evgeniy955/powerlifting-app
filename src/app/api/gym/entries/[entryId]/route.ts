...
// PATCH { oneRepMax } — unchanged: sets this entry's (and the client's
// tracked) 1RM for its exercise.
...
export async function PATCH(req: Request, { params }: { params: Promise<{ entryId: string }> }) {
  try {
    ...
    const body = await req.json() as { oneRepMax?: unknown; exerciseId?: string }

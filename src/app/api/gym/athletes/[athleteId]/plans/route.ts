...
export async function POST(req: Request, { params }: { params: Promise<{ athleteId: string }> }) {
  ...
  const plan = await prisma.gymPlan.create({ data: { clientId, name, weeks, startDate: new Date(body.startDate ?? Date.now()), weeksData: { create: Array.from({ length: weeks }, (_, i) => ({ weekNumber: i + 1, workouts: { create: Array.from({ length: 3 }, (_, d) => ({ dayNumber: d + 1, scheduledDate: new Date(Date.now() + (i * 7 + d) * 86400000) })) } })) } } })

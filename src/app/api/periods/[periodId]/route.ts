...
// (schema onDelete: Cascade), which in turn detach (not delete) any Cycles
// attached to them — real training plans are never deleted by removing a
// period/stage, only unlinked from the timeline (Cycle.stageId -> null via
// onDelete: SetNull on Stage -> Cycle).
